...
from pact.message_consumer_v3 import MessageConsumerV3, Provider
from pact.message_pact_v3 import MessagePactV3
from pact.constants import MESSAGE_PATH
from pact.matchers_v3 import Regex
from pact import message_pact_v3

class MessagePactV3TestCase(TestCase):
    def setUp(self):
        self.consumer = MessageConsumerV3('TestConsumer')
        self.provider = Provider('TestProvider')

    def test_init_defaults(self):
        target = MessagePactV3(self.consumer, self.provider)
    ...
    def test_definition_sparse(self):
        ...
        (
            ...
            .with_metadata({'contentType': 'application/json',
                            'source': 'legacy_api',
                            'some-header': Regex('\\d+-\\d+-\\d+T\\d+:\\d+:\\d+', '2022-02-15T20:16:01')})
        )
...
class MessagePactV3ContextManagerTestCase(MessagePactV3TestCase):
    def setUp(self):
        super(MessagePactV3ContextManagerTestCase, self).setUp()
        self.addCleanup(patch.stopall)

        self.write_to_pact_file = patch.object(
            message_pact_v3.MessagePactV3, 'write_to_pact_file', autospec=True).start()
        self.write_to_pact_file.return_value.returncode = 0

        self.mock_publish = patch.object(
            message_pact_v3.MessagePactV3, 'publish', autospec=True).start()
...
class PactGeneratePactFileTestCase(TestCase):
    def setUp(self):
        ...
        self.addCleanup(patch.stopall)
        self.mock_Popen = patch.object(message_pact_v3, 'Popen', autospec=True).start()
